...
# アプリケーション概要
シフト希望・シフト管理を一つのアプリケーションで行い、いつでもどこでも最新のシフトを確認できます。
...
# 工夫したポイント
工夫したのは、シフトを作成する側のサポートとして、シフト希望の内容をシフト作成画面に連動させた上で、シフト作成画面を修正した時にシフト希望側には修正したことが反映されないようにする部分です。
またシフトを見やすくする為にカレンダー方式にする為にsimple_calendarのGemを使用した事です。
このアプリを使用すれば、いつでもどこでも最新のシフトを確認できるので良いと思います。
